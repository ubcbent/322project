...
public class minimaxNode {
	...
	private LegalMove moveLegal;
	private minimaxNode parentNode;
	private ArrayList<minimaxNode> childNodes = new ArrayList<minimaxNode>();
	private int[][] tempboard;
	private int hVal;

	minimaxNode(minimaxNode par, LegalMove legal, int[][] board) {
		moveLegal = legal;
		parentNode = par;
		childNodes = new ArrayList<minimaxNode>();
		tempboard = board;
		hVal = 0;
	}
	
	public void setLegalMove(LegalMove move) {
		this.moveLegal = move;
	}
	
	public LegalMove getLegalMove() {
		return this.moveLegal;
	}
	...
	public void addChildren(ArrayList<minimaxNode> children) {
		for(minimaxNode c : children) {
			c.setParent(this);
		}
	...
	public void setTempBoard(int[][] board) {
...
/*	public void getLegalMoves() {
	...
	protected void checkArrowMoves(int currRow, int currCol, int newRow, int newCol) {
		// for checking arrow shot candidates, we know we may shoot an arrow through the
		// spot we moved from, or through any empty spot
		// horizontal moves
		for (int left = 1; newCol - left > 0; left++) {// check spaces to left of queen
			if (tempboard[newRow][newCol - left] == 0 || (newRow == currRow && newCol - left == currCol)) { // space is
																											// clear
		...
		for (int right = 1; newCol + right <= 10; right++) {// check spaces to right of queen
			if (tempboard[newRow][newCol + right] == 0 || (newRow == currRow && newCol + right == currCol)) { // space
																												// is
																												// clear
				int[] curr = { currRow, currCol };
				int[] neww = { newRow, newCol };
		...
		for (int down = 1; newRow - down > 0; down++) {// check spaces under queen
			if (tempboard[newRow - down][newCol] == 0 || (newRow - down == currRow && newCol == currCol)) { // space is
																											// clear
				int[] curr = { currRow, currCol };
				int[] neww = { newRow, newCol };
		...
		for (int up = 1; newRow + up <= 10; up++) {// check spaces above queen
			if (tempboard[newRow + up][newCol] == 0 || (newRow + up == currRow && newCol == currCol)) { // space is
																										// clear
				int[] curr = { currRow, currCol };
				int[] neww = { newRow, newCol };
		...
		for (int dl = 1; newRow - dl > 0 && newCol - dl > 0; dl++) {// check spaces down and to the left of queen
			if (tempboard[newRow - dl][newCol - dl] == 0 || (newRow - dl == currRow && newCol - dl == currCol)) { // space
																													// is
																													// clear
				int[] curr = { currRow, currCol };
				int[] neww = { newRow, newCol };
		...
		for (int ur = 1; newRow + ur <= 10 && newCol + ur <= 10; ur++) {// check spaces up and to the right of queen
			if (tempboard[newRow + ur][newCol + ur] == 0 || (newRow + ur == currRow && newCol + ur == currCol)) { // space
																													// is
																													// clear
				int[] curr = { currRow, currCol };
				int[] neww = { newRow, newCol };
		...
		for (int ul = 1; newRow + ul <= 10 && newCol - ul > 0; ul++) {// check spaces up and to the left of queen
			if (tempboard[newRow + ul][newCol - ul] == 0 || (newRow + ul == currRow && newCol - ul == currCol)) { // space
																													// is
																													// clear
				int[] curr = { currRow, currCol };
				int[] neww = { newRow, newCol };
		...
		for (int dr = 1; newRow - dr > 0 && newCol + dr <= 10; dr++) {// check spaces down and to the left of queen
			if (tempboard[newRow - dr][newCol + dr] == 0 || (newRow - dr == currRow && newCol + dr == currCol)) { // space
																													// is
																													// clear
				int[] curr = { currRow, currCol };
				int[] neww = { newRow, newCol };
